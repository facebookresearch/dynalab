...
class Handler(BaseDynaHandler):
    ...
    def preprocess(self, data):
        ...
        input_data = self.tokenizer.encode_plus(*args,  max_length=512, return_tensors="pt")
    ...
    def postprocess(self, inference_output, data):
        ...
        if len(inference_output) == 2:
            response["prob"] = {"entailed": float(inference_output[0]), "contradictory": float(inference_output[1]), "neutral": float(0)}
        else:
